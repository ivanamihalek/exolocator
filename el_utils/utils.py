...
# compressed files do not have zero size on disk
def is_gz_empty(file_path):
    with gzip.open(file_path, 'rb') as gz_file:
        # check if the first byte is empty
        return gz_file.read(1) == b''  # Returns True if no data in the gz file


def count_lines_in_compressed_file(filepath):
    cmd = f'zgrep -Ec "$" {filepath}'
...
def pairwise_tanimoto(seq1, seq2, use_heuristics=True):
    ...
    if not l1 or not l2: return tanimoto
...
if __name__ == "__main__":
    print(count_lines_in_compressed_file("/media/ivana/portable/ensembl-113/mysql/ensembl_compara_113/homology.txt.gz"))
