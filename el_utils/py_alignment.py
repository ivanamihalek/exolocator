...
#     digits   = list(map(chr, list(range(48, 58))))
#
...
#             if ((any1, any2) not in matrix):
#                 if ((any2, any1) not in matrix):
#                     matrix[(any1, any2)] = 0
#                 else:
...
#     F         = [[0.0]*(max_j+1) for x in range(max_i+1)]
#     direction = [[''] *(max_j+1) for x in range(max_i+1)]
...
#
#     almt = ""
#     sim_matrix  = [[0.0]*len(ortho_exons) for x in range(len(human_exons) )]
#     aligned_pep_seqs  = [[ ["",""] ]*len(ortho_exons) for x in range(len(human_exons) )]
#
