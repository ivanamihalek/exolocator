...
# the input is of the form [-20,+3] sequence around the splice position
...
while(<FILE>) {
    ...
    $maxent_score = maxentscore($str2, \@metables);
...
sub makemaxentscores{
    my $dir = "$me_homedir/splicemodels/";
...
sub scoreconsensus{
  ...
  my $denominator = $bgd{$seqa[18]} * $bgd{$seqa[19]};
  $denominator || return 0.0;
  my $addscore = $cons1{$seqa[18]} * $cons2{$seqa[19]}/$denominator;
  return $addscore;
}
sub log2{
    my ($val) = @_;
