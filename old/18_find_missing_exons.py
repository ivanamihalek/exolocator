...
def search_for_exons(human_gene_list, db_info):
    ...
    if verbose: print "in search_for_exons()"
	...
	human_exons.sort(key=lambda exon: exon.start_in_gene)
        ...
        index = {}
        for idx, he in enumerate(human_exons):
            index[he] = idx
	...
	maps  = {}
        for he in human_exons:
           ...
           # should delete later:
           maps[he] = filter (lambda m: m.similarity > 0.7 and not m.source == 'sw_sharp',  maps_for_exon)
           print
           print he.exon_id
           for m in maps[he]:
               if not m.species_2 == 'macropus_eugenii': continue
               human_exon_pepseq  = get_exon_pepseq (cursor, he, ensembl_db_name['homo_sapiens'])
               if not human_exon_pepseq: continue
               [exon_seq_id, protein_seq, pepseq_transl_start, pepseq_transl_end, 
                left_flank, right_flank, dna_seq] = get_exon_seqs(cursor, m.exon_id_2, 
                                                                  m.exon_known_2, ensembl_db_name[m.species_2])
               print human_exon_pepseq
               print protein_seq
	...
	for species in ['macropus_eugenii']:
		...
		for gene_id  in regions[species].keys():
			...
			nearestseqs = [get_exon_seqs(cursor, m.exon_id_2, m.exon_known_2, ensembl_db_name[species2]) + [m.exon_id_2]
					for m in maps[he] if species2 == m.species_2 and m.similarity > 0.7]
                        nearestseqs = filter (lambda seq: len(seq)>=8, nearestseqs)
			if nearestseqs:
			    nearestspecies = species2
			    break

                    if not nearestspecies: continue
                    [exon_seq_id, protein_seq, pepseq_transl_start, pepseq_transl_end, left_flank, right_flank, dna_seq, template_exon_id] = nearestseqs[0]
